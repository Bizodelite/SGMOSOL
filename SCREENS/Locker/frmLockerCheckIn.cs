    ...
    public partial class frmLockerCheckIn : Form
    {
        ...
        private Collection GetCheckInDetColl()
        {
            ...
            for (var i = 0; i <= chkLockers.Items.Count - 1; i++)
            {
                if (chkLockers.GetItemChecked(i))
                {
                    ...
                    CheckInDet.LockerRecordModifiedCount = CF.lsbItemName2(chkLockers, i) + 1;
                    coll.Add(CheckInDet);
                }
            }
            return coll;
        ...
        private bool LoadTransaction(long lngSearchId,bool blnLockForm)
        {
            ...
            try
            {
                ...
                {
                    ctr++;
                    chkLockers.Items.Add(new clsItemData(row["LockerName"].ToString(), Convert.ToInt32(row["LockerId"]), Convert.ToInt32(row["RecordModifiedCount"])));
