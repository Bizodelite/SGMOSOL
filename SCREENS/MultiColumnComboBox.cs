...
namespace SGMOSOL.SCREENS
{
    public class MultiColumnComboBox : ComboBox
    {
        ...
        {
            base.OnDropDown(e);
            this.DropDownWidth = System.Convert.ToInt32(Math.Truncate(CalculateTotalWidth()));
        }
        ...
        private void InitializeColumns()
        {
            ...
            columnWidths = new float[propertyDescriptorCollection.Count - 1 + 1];
            columnNames = new String[propertyDescriptorCollection.Count - 1 + 1];

            for (int colIndex = 0; colIndex <= propertyDescriptorCollection.Count - 1; colIndex++)
            {
        ...
        protected override void OnMeasureItem(MeasureItemEventArgs e)
        {
            ...
            for (int colIndex = 0; colIndex <= columnNames.Length - 1; colIndex++)
            {
            ...
            e.ItemWidth = System.Convert.ToInt32(Math.Truncate(totWidth));
        }
                ...
                using (SolidBrush brush = new SolidBrush(ForeColor))
                {
                    if (columnNames.Length == 0)
                        ...
                        for (int colIndex = 0; colIndex <= columnNames.Length - 1; colIndex++)
                        {
                            ...
                            boundsRect.Width = System.Convert.ToInt32(Math.Truncate(columnWidths[colIndex])) + columnPadding;
